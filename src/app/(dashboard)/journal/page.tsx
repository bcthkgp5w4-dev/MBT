...
export default function JournalPage() {
  ...
  async function startAudioRecording() {
    if (recordingAudio) { stopCurrentRecording(); setRecordingAudio(false); return }
    try {
      ...
      mr.onstop = () => {
        ...
        setPendingMedia(prev => [...prev, { file, previewUrl: URL.createObjectURL(blob), type: 'audio' }])
  ...
  async function startVideoRecording() {
    if (recordingVideo) { stopCurrentRecording(); setRecordingVideo(false); return }
    try {
      ...
      mr.onstop = () => {
        ...
        setPendingMedia(prev => [...prev, { file, previewUrl: URL.createObjectURL(blob), type: 'video' }])
  ...
  function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    Array.from(e.target.files || []).forEach(file => {
      setPendingMedia(prev => [...prev, { file, previewUrl: URL.createObjectURL(file), type: file.type.startsWith('video') ? 'video' : 'audio' }])
    })
    e.target.value = ''
  }

  function removeMedia(index: number) {
    setPendingMedia(prev => { URL.revokeObjectURL(prev[index].previewUrl); return prev.filter((_, i) => i !== index) })
  }
  ...
  async function uploadMediaFiles(userId: string, entryId: string): Promise<string[]> {
    ...
    for (const item of pendingMedia) {
      ...
      if (!error) urls.push(supabase.storage.from('journal-media').getPublicUrl(path).data.publicUrl)
    }
    return urls
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) return
      const { data: entry, error } = await supabase.from('journal_entries').insert({ child_id: selectedChild, profile_id: user.id, content: newEntry, mood }).select().single()
      if (error) throw error
      let mediaUrls: string[] = []
      if (pendingMedia.length > 0) {
        mediaUrls = await uploadMediaFiles(user.id, entry.id)
        if (mediaUrls.length > 0) await supabase.from('journal_entries').update({ media_urls: mediaUrls }).eq('id', entry.id)
      }
      setEntries(prev => [{ ...entry, media_urls: mediaUrls }, ...prev])
      setNewEntry('')
      setPendingMedia([])
      fetch('/api/journal/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ entryId: entry.id }) })
    } finally {
      setSubmitting(false)
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
        <form onSubmit={handleSubmit}>
          ...
          <textarea value={newEntry} onChange={e => setNewEntry(e.target.value)}
            ...
            rows={4} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none mb-3" />
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <button type="button" onClick={startAudioRecording}
              className={cn('flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg transition-colors',
                recordingAudio ? 'bg-red-100 text-red-600 ring-2 ring-red-400 animate-pulse' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}>
            ...
            <button type="button" onClick={startVideoRecording}
              className={cn('flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg transition-colors',
                recordingVideo ? 'bg-red-100 text-red-600 ring-2 ring-red-400 animate-pulse' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}>
            ...
            <input ref={fileInputRef} type="file" accept="audio/*,video/*" multiple className="hidden" onChange={handleFileUpload} />
          </div>
            ...
            <div className="space-y-2 mb-3">
              {pendingMedia.map((item, i) => (
                <div key={i} className="flex items-center gap-2 bg-gray-50 rounded-xl p-2">
                  ...
                  <button type="button" onClick={() => removeMedia(i)} className="text-gray-400 hover:text-red-500 p-1 shrink-0">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ))}
            ...
            <div className="flex gap-2">
              ...
              {Object.entries(MOOD_CONFIG).map(([key, config]) => (
                <button key={key} type="button" onClick={() => setMood(key)}
                  className={cn('text-lg p-1.5 rounded-lg transition-colors', mood === key ? 'bg-blue-100 ring-2 ring-blue-500' : 'hover:bg-gray-50')}
                  title={config.label}>{config.emoji}</button>
              ))}
            </div>
            <button type="submit" disabled={submitting || !newEntry.trim() || !selectedChild}
              ...
              {submitting ? <><Loader2 className="w-4 h-4 animate-spin" />{uploadingMedia ? 'Uploading…' : 'Saving…'}</> : <><Plus className="w-4 h-4" />Add Entry</>}
            </button>
          </div>
        </form>
      </div>
            ...
            return (
              <div key={entry.id} className="bg-white rounded-2xl border border-gray-100 p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {moodConfig && <span className={cn('text-xs font-medium px-2 py-1 rounded-full', moodConfig.color)}>{moodConfig.emoji} {moodConfig.label}</span>}
                </div>
                  ...
                  <div className="mt-3 space-y-2">
                    {entry.media_urls.map((url, i) => (
                      <div key={i} className="rounded-xl overflow-hidden bg-gray-50 p-2">
                        {isVideo(url) ? <video src={url} controls className="w-full max-h-56 rounded-lg" /> : <audio src={url} controls className="w-full h-10" />}
                      </div>
                    ))}
                  </div>
                )}
                {entry.ai_analysis && (
                  <div className="mt-4 bg-purple-50 rounded-xl p-4 border border-purple-100">
                    <p className="text-xs font-medium text-purple-600 mb-2 flex items-center gap-1"><Sparkles className="w-3 h-3" />AI Insights</p>
                    <p className="text-sm text-purple-800">{entry.ai_analysis}</p>
                    {entry.ai_recommendations && entry.ai_recommendations.length > 0 && (
                      <div className="mt-2">{entry.ai_recommendations.slice(0, 2).map((rec: string, i: number) => <p key={i} className="text-xs text-purple-700 mt-1">• {rec}</p>)}</div>
                    )}
                  </div>
                )}
              </div>
